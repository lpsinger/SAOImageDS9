...
proc FPSelectCmd {varname ss rc} {
    ...
    if {$var(edit)} {
	FPSelectEditCmd $varname $ss $rc
    } else {
	FPSelectBrowseCmd $varname $ss $rc
    }
}

proc FPSelectEditCmd {varname ss rc} {
    upvar #0 $varname var
    global $varname

    global debug
    if {$debug(tcl,footprint)} {
	puts stderr "FPSelectEditCmd $varname $rc"
    }
...
proc FPEditCB {tag id} {
    global debug
    if {$debug(tcl,footprint)} {
	puts stderr "FPEditCB $tag $id"
    }
...
proc FPMoveCB {tag id} {
    global debug
    if {$debug(tcl,footprint)} {
	puts stderr "FPMoveCB $tag $id"
    }
...
proc FPRotateCB {tag id} {
    global debug
    if {$debug(tcl,footprint)} {
	puts stderr "FPRotateCB $tag $id"
    }
